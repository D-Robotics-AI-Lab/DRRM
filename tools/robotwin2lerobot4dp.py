...
FEATURES = {
    "head_camera": {"dtype": "video", "shape": (240, 320, 3), "names": ["h", "w", "c"]},
...
def _process_episode(dataset: LeRobotDataset, episode_dir: Path, episode_idx: int) -> None:
...
def main(args):
    ...
    for episode_idx in range(len([d for d in os.listdir(args.src) if os.path.isdir(os.path.join(args.src, d))])):
        episode_dir = Path(os.path.join(args.src, f"episode{episode_idx}"))

        assert os.path.isdir(episode_dir), f"Episode directory {episode_dir} does not exist"

        _process_episode(ds, episode_dir, episode_idx)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--src", type=str, required=True, help="Source directory containing episode folders, robotwin format")
